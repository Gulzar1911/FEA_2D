function [K_mod, F_mod] = applyBoundaryConditions(K_global, F_global, fixedNodes, loadedNodes, forceMagnitude)
    K_mod = K_global;
    F_mod = F_global;
...
for node = loadedNodes
    F_mod((node-1)*dof + 1) = F_mod((node-1)*dof + 1) + forceMagnitude; % Apply force in X-direction
end
